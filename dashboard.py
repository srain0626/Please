...
<body>
  <div class="container">
    ...
    <section>
      <h2>관리 액션</h2>
      <form method="post" action="/action/snapshot_kpi" class="inline">
        <button type="submit">KPI Snapshot 생성</button>
      </form>
      <br />
      <form method="post" action="/action/update_intent_status" class="inline">
        ...
        <label>Status</label>
        <select name="status">
          <option>submitted</option><option>partial_fill</option><option>filled</option><option>canceled</option><option>rejected</option>
        </select>
      ...
      <form method="post" action="/action/update_position_status" class="inline">
        ...
        <label>Status</label>
        <select name="status">
          <option>submitted</option><option>open</option><option>closed</option><option>canceled</option>
        </select>
    ...
    <div class="grid">
      <section>
        <h2>Team KPI (실시간 집계)</h2>
        {_table(['assignee', 'task_count', 'revenue', 'cost', 'profit'], kpi_rows)}
      </section>
      <section>
        <h2>최근 시스템 이벤트</h2>
        {_table(['id', 'event_type', 'details', 'created_at'], events)}
      </section>
    </div>
...
def create_handler(store: SQLiteStore):
    class DashboardHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if parsed.path == "/action/update_intent_status":
                store.update_order_intent_status(form.get("client_order_id", ""), form.get("status", "submitted"))
                self._redirect("Intent status updated")
                return
            if parsed.path == "/action/update_position_status":
                try:
                    ...
                    position_id = 0
                if position_id > 0:
                    store.update_position_status(position_id, form.get("status", "submitted"))
                self._redirect("Position status updated")
                return
...
def main() -> None:
    ...
    print(f"Dashboard running on http://{args.host}:{args.port} (db={args.db_path})")
